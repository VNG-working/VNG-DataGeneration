...
class CMSQ_BACK(OfficialID):
    ...
    def fakeMainInfo(self):
        ...
        self.get_marker_coord(
            text, [text], ["marker_dob"], self.Bold)
        ...
        text = self.write(text, self.normal)
        ...
        text = self.write(text, self.Bold)
        self.get_marker_coord(text, [text], ['marker_ethnic'], self.Bold)
        ...
        text = self.write(text, self.normal)
        self.get_field_coord(text, [text], ['ethnic'], self.normal)
        ...
        text = self.write(text, char_font=self.Bold, ink=randink(True))
        self.get_marker_coord(
            text, [text], ["marker_hometown"], self.Bold)
        ...
        self.get_field_coord(text, [text], ["hometown"], self.normal)
        
        # que quan part 2
        center_x = self.cursor[0] + self.get_text_length(text, self.normal) // 2
        text = f"{self.district}, {self.province}."
        ...
        text = self.write(text, char_font=self.Bold, ink=randink(True))
        self.get_marker_coord(
            text, [text], ["marker_address"], self.Bold)
        ...
        text = self.write(text, self.normal)
        self.get_field_coord(text, [text], ['address'], self.normal)
        ...
        text = self.write(text, self.Bold)
        self.get_marker_coord(text, [text], ['marker_characteristic'], self.Bold)
        ...
        if np.random.rand() < 0.5: # cao 1m7 dong tren, dac diem dong duoi
            ...
            text = self.write(text, self.normal)
            self.get_field_coord(text, [text], ['characteristic'], self.normal)
        ...
        text = self.write(text, self.Bold)
        self.get_marker_coord(text, [text], ['marker_blood'], self.Bold)
        ...
        text = self.write(text, self.Bold)
        self.get_field_coord(text, [text], ['blood'], self.Bold)
