...
if __name__ == '__main__':
    ...
    while i < args.count:
        try:
            order_type = np.random.choice(['up|down', 'left|right'], p=[0.5, 0.5])
            if order_type == 'up|down':
                partyA = Party(skip_prob = 0.1, down_prob = 0.6, bank_prob = 0.8, font_size=font_size)()
                partyB = Party(skip_prob = 0.1, down_prob = 0.6, bank_prob = 0.8, font_size=font_size)()
            else:
            ...
            modules.append(bank)
            out_dir = '/home/fiores/Desktop/VNG/VNG-DataGeneration/contract/result_block'
            for module in modules:
                if module.__class__.__name__ == 'Bank':
                    module.canvas.save(f'{out_dir}/bank_{i}.png')
                elif module.__class__.__name__ == 'Party':
                    module.canvas.save(f'{out_dir}/party_{i}.png')
