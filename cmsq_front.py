...
class CMSQ_FRONT(OfficialID):
    ...
    def fakeMainInfo(self):
        ...
        raw_text = "GIẤY CHỨNG MINH SĨ QUAN"
        text = raw_text
        self.Bold_huge = ImageFont.truetype(
            "fonts/Arial/ARIALBD.ttf", size=self.font_scale+randint(10, 20))
        self.write(text, char_font=self.Bold_huge, ink=(187+randint(-20, 20),43+randint(-20, 20),33+randint(-20, 20)))
        self.get_field_coord(text, [raw_text], ["outlier"], self.Bold_huge)
        ...
        self.get_marker_coord(text, [raw_text], ["marker_number"], self.Bold)
        ...
        text = raw_text
        self.write(text, char_font=self.Bold, ink=randink(True))
        self.get_marker_coord(text, [raw_text], ["number"], self.Bold)
        ...
        text = raw_text
        self.write(text, char_font=self.Bold, ink=randink(True))
        self.get_marker_coord(text, [raw_text], ["marker_name"], self.Bold)
        ...
        raw_text = name
        text = raw_text
        self.write(text, char_font=self.Bold, ink=randink(extra_bold=True))
        ...
        text = raw_text
        self.write(text, char_font=self.Bold, ink=randink(True))
        self.get_marker_coord(text, [raw_text], ["marker_ranking"], self.Bold)
        ...
        raw_text = capbac
        text = raw_text
        self.write(text, char_font=self.semi_bold, ink = randink())
        self.get_field_coord(text, [raw_text], ["ranking"], self.semi_bold)
        ...
        self.get_marker_coord(
            text, [raw_text], ["marker_pow"], self.Bold)
        ...
        text = raw_text
        self.write(text, char_font=self.semi_bold, ink = randink())
        self.get_field_coord(text, [raw_text], ["pow"], self.semi_bold)
        ...
        if np.random.rand() < 0.5:
            ...
            ink = randink()
        self.write(text, self.italic, ink=ink)
        ...
        self.cursor[1] += self.line_height + randint(-10, 10)
        self.write(text, char_font=self.Small_Bold, ink = randink(True))
        self.get_field_coord(text, [raw_text], ["signer_position"], self.Small_Bold)
