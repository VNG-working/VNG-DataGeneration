...
class CMCA(OfficialID):
    ...
    def fakeMainInfo(self):
        ...
        text = self.write(text, font, ink=randink(bold=True))
        self.get_field_coord(text, [text], ['outlier'], font)
        ...
        text = "GIẤY CHỨNG MINH CÔNG AN NHÂN DÂN"
        
        font = ImageFont.truetype('fonts/HeadingNowTrial-56Bold.ttf', self.font_scale+randint(15, 25))
        text = self.write(text, char_font=font, ink=(182, 31, 34))
        self.get_field_coord(text, [text], ["outlier"], font)
        ...
        text = self.write(text, char_font=self.normal, bold=True)
        self.get_marker_coord(text, [text], ["marker_number"], self.normal)
        
        # actual so
        self.cursor[0] += self.get_text_length(text, self.normal) + randint(30, 70)
        text = random_number(3) + np.random.choice(['  -  ', ' - ', '- ', ' -']) + random_number(3)
        
        text = self.write(text, char_font=self.big_bold, ink=randink(True))
        self.get_field_coord(text, [text], ["number"], self.big_bold)
        self.merge_bb('number')
        ...
        text = self.write(text, char_font=self.normal, ink=randink(True))
        self.get_marker_coord(text, [text], ["marker_name"], self.normal)
        ...
        text = self.write(text, char_font=self.bold, ink=randink(bold=True))
        ...
        text = self.write(text, char_font=self.normal, bold=True)
        self.get_marker_coord(text, [text], ["marker_dob"], self.normal)
        ...
        text = self.write(text, char_font=self.bold, ink=randink(bold=True))
        self.get_field_coord(text, [text], ["dob"], self.bold)
        ...
        text = self.write(text, char_font=self.normal, ink=randink(True))
        self.get_marker_coord(text, [text], ["marker_rank"], self.normal)
        ...
        self.get_field_coord(text, [text], ["rank"], self.bold)
        ...
        text = self.write(text, char_font=self.normal, ink=randink(True))
        self.get_marker_coord(text, [text], ["marker_position"], self.normal)
        ...
        text = self.write(text, char_font=self.bold, ink=randink(True))
        self.get_field_coord(text, [text], ["position"], self.bold)
        ...
        text = self.write(text, char_font=self.normal, ink=randink(True))
        self.get_marker_coord(text, [text], ["marker_pow"], self.normal)
        ...
        text = self.write(text, char_font=self.roboto, ink=randink(True))
        self.get_field_coord(text, [text], ["pow"], self.roboto)
        ...
        if np.random.rand() < 0.8:
            ...
            self.cursor[1] += self.line_height + randint(-10, 10)
            text = self.write(text, char_font=self.italic, bold=True)
            self.get_field_coord(text, [date, month, year], ["doi", 'doi', 'doi'], self.italic)
        else:
            text = f'{self.province}, ngày {date}/{month}/{year}'
            self.cursor[0] = randint(1050, 1350)
            self.cursor[1] += self.line_height + randint(-10, 10)
            text = self.write(text, char_font=self.italic, bold=True)
            self.get_field_coord(text, [date, month, year], ["doi", 'doi', 'doi'], self.italic)
        ...
        self.cursor[1] += self.line_height + randint(-10, 10)
        text = self.write(text, char_font=font, ink=randink(bold=True))
        self.get_field_coord(text, [text], ["signer_position"], font)
        ...
        text = self.write(text, char_font=self.normal, ink=randink(True))
        self.get_marker_coord(text, [text], ["marker_blood"], self.normal)
        ...
        text = self.write(text, char_font=self.Bold_BLOOD, ink=randink(True))
        self.get_field_coord(text, [text], ["blood"], self.Bold_BLOOD)
