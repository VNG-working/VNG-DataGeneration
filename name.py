...
class Buyer(Module):
    ...
    def __call__(self, submodules: list):
        for sub_module in submodules:
            ...
            self.__paste__(submodule=sub_module, position=(100, 100))
...
if __name__ == "__main__":
    ...
    #     start_point = (point["xmin"], point['ymin'])
    #     end_point = (point["xmax"], point['ymax'])
    #     test.canvas = cv2.rectangle(test.canvas, start_point, end_point, (0, 0, 255), thickness = 2)

    # cv.imshow("test", test.canvas)
    # cv.waitKey(0)

    buyer_test = Buyer(shape = (3000, 3000))
    print(buyer_test.get_shape())
    buyer_test([test])
