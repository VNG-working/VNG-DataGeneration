...
class CMQD_Back(OfficialID):
    ...
    def fakeMainInfo(self):
        ...
        self.get_marker_coord(text, [text], ["marker_hometown"], self.normal)
        ...
        if np.random.rand() < 0.4 or len(hometown) > 45:
            ...
            self.get_field_coord(text, [text], ['hometown'], self.bold)
        ...
        self.get_marker_coord(text, [text], ['marker_address'], self.normal)
        ...
        if np.random.rand() < 0.4 or len(address) > 35:
            ...
            text = self.write(text, self.bold)
            self.get_field_coord(text, [text], ['address'], self.bold)
        ...
        text = self.write(text, self.normal)
        self.get_marker_coord(text, [text], ['marker_characteristic'], self.normal)
        ...
        if np.random.rand() < 0.5: # cao 1m7 dong tren, dac diem dong duoi
            ...
            text = self.write(text, self.bold)
            self.get_field_coord(text, [text], ['characteristic'], self.bold)
